...
- This repo ships optional remote-control helpers under `scripts/`.
...
- Keep machine names, login details, queue names, container tags, and host
  topology out of library modules, packaged configs, and default workflows.
...
- The scripts use a repo-local `.remote-known-hosts` file for non-interactive
  access. Any manual SSH aliases belong in the user's `~/.ssh/config`, not in
  the repo contract.
