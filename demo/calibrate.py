...
def velocity_strike_pair(
    ...
    port.send(mido.Message("note_on", note=note_high_vel, velocity=high_velocity))
    ...
    try:
        with mido.open_output(port_name) as port:
            ...
            while True:
                ...
                cmd = input("[u]p / [d]own / [r]epeat / [q]uit: ").strip().lower()
...
def calibrate_note_timing(
    ...
    try:
        with mido.open_output(port_name) as port:
            print(f"Opened MIDI output: {port_name}\n")
            while True:
                note_repetition_trial(port, velocity, notes, note_length_ms, gap_ms)
...
def parse_args():
    ...
    parent.add_argument(
        ...
        default=120,
        help=(
            "Note-on velocity (1-127). For 'velocity' mode, this is the HIGH velocity."
        ),
    )
    parent.add_argument(
        "--step",
