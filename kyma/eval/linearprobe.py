"""Embedding export and linear-probe evaluation helpers."""
...
def get_chunks(note_msgs: list, chunk_len: int):
    return [note_msgs[i : i + chunk_len] for i in range(0, len(note_msgs), chunk_len)]
...
def process_entry(
    ...
    if isinstance(entry, dict):
        midi_dict = MidiDict.from_msg_dict(cast(Any, entry))
    else:
        midi_dict = entry
...
def get_aria_contrastive_embedding(
    ...
    for i in range(0, len(seqs), hook_max_batch_size):
        ...
        padded_seqs = [
            _pad_seq(seq=seq, tokenizer=hook_tokenizer, max_seq_len=hook_max_seq_len)
            for seq in batch_seqs
        ]
        eos_positions = [seq.index(hook_tokenizer.eos_tok) for seq in padded_seqs]
...
def get_mert_embedding(
    ...
    from kyma.eval.mert.emb import (
...
def get_clamp3_embedding(
    ...
    from kyma.eval.m3.emb import get_midi_embedding

    emb = [
        get_midi_embedding(
            mid=hook_tokenizer.detokenize(seq).to_midi(),
            model=cast(Any, hook_model),
...
def get_baseline_embedding(
    ...
    seqs = [
        seq + ([hook_tokenizer.pad_tok] * (hook_max_seq_len - len(seq))) for seq in seqs
    ]

    enc_seqs = torch.tensor([hook_tokenizer.encode(seq) for seq in seqs], device="cuda")
...
class EvaluationDataset(torch.utils.data.Dataset):
    def __init__(self, load_path: str, tag_to_id: dict, metadata_category: str):
        ...
        if self.mmap_obj is None:
            raise RuntimeError("Failed to open the evaluation dataset.")
    ...
    def reopen(self) -> None:
        self.close()
        self.file_buff = self._resources.enter_context(
            _openbinarybuffer(self.load_path)
        )
        self.mmap_obj = mmap.mmap(self.file_buff.fileno(), 0, access=mmap.ACCESS_READ)
    ...
    def __getitem__(self, idx: int):
        ...
        if tag not in self.tag_to_id:
            raise ValueError(f"Unknown tag {tag!r} for metadata {metadata!r}.")
        ...
        **embedding_hook_kwargs,
        ...
        def batch_producer(
            ...
            while termination_signals < total_workers:
                ...
                if per_file:
                    if not all("abs_load_path" in r["metadata"] for r in result):
                        raise ValueError("Per-file embeddings require abs_load_path.")
                    buffer.extend(result)
                    if len(buffer) > 2 * batch_size:
                        print(
                            "WARNING: Generated batch of size "
                            f"{len(buffer)} (batch_size={batch_size})"
                        )
        ...
        if not os.path.isfile(midi_dataset_load_path):
            raise FileNotFoundError(f"MIDI dataset not found: {midi_dataset_load_path}")
            ...
            while batch_producer_process.is_alive() or not batch_queue.empty():
                try:
                    ...
                    if not per_file_embeddings:
                        ...
                        write_objs = []
                        for _file_path, data in groups.items():
                            avg_emb = torch.tensor(data["embs"]).mean(dim=0).tolist()
                            write_objs.append(
                                {
                                    "seqs": data["seqs"],
                                    "emb": avg_emb,
...
def _train(
    ...
    for _epoch in range(num_epochs):
        for __step, batch in (pbar := tqdm(enumerate(train_dataloader), leave=False)):
            ...
            if scheduler:
                scheduler.step()
                lr_for_print = f"{scheduler.get_last_lr()[0]:.2e}"
