"""Utility layers and data transforms for the optional M3 evaluators."""
    ...
    def gather_features(
        ...
        if not has_distributed:
            raise RuntimeError(
                ...
                "use a PyTorch build with distributed support."
            )
        if use_horovod:
            if hvd is None:
                raise RuntimeError("Please install horovod for this code path.")
            if gather_with_grad:
                ...
                with torch.no_grad():
                    ...
                    all_text_features = hvd.allgather(text_features)
                if not local_loss:
                    # Preserve local-rank gradients when gathered features are detached.
...
class M3Patchilizer:
    ...
    def encode(
        ...
        if len(patches) > PATCH_LENGTH and truncate:
            ...
            if choice == "head" or not random_truncate:
...
class M3TokenDecoder(PreTrainedModel):
    ...
    def forward(
        self,
        patch_features,
        target_patches,  # [batch_size, hidden_size]
    ):  # [batch_size, seq_length]
...
class CLaMP3Model(PreTrainedModel):
    def __init__(
        ...
        if global_rank is None or world_size is None:
            global_rank = 0
            world_size = 1
        ...
        if load_m3 and os.path.exists(M3_WEIGHTS_PATH):
            ...
            model = None
            print(
                "Successfully loaded M3 checkpoint from "
                f"epoch {checkpoint['epoch']} with loss {checkpoint['min_eval_loss']}"
            )
...
def remove_instrument_info(item):
    ...
    type = "mtf" if lines[0].split(" ")[0] == "ticks_per_beat" else "abc"
